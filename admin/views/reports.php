...
<?php else : ?>
	<div class="sabri-feed-report-list">
		<?php foreach ( $items as $report ) : ?>
			<?php $allowed_targets = \Sabri\HomeNewsFeed\ReportPolicy::transition_targets( $report['status'] ); ?>
			<article class="sabri-feed-report-card">
				<header><h2><?php echo esc_html( sprintf( __( 'Report #%d', 'sabri-complete-home-news-feed' ), (int) $report['id'] ) ); ?></h2><span class="sabri-feed-report-status"><?php echo esc_html( isset( $state_labels[ $report['status'] ] ) ? $state_labels[ $report['status'] ] : $report['status'] ); ?></span></header>
				<dl>
					<dt><?php esc_html_e( 'Reported content', 'sabri-complete-home-news-feed' ); ?></dt><dd><?php if ( ! empty( $report['object_url'] ) ) : ?><a href="<?php echo esc_url( $report['object_url'] ); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html( $report['object_label'] ); ?></a><?php else : ?><?php echo esc_html( $report['object_label'] ); ?><?php endif; ?><?php if ( ! empty( $report['object_excerpt'] ) ) : ?><blockquote><?php echo esc_html( $report['object_excerpt'] ); ?></blockquote><?php endif; ?></dd>
					<dt><?php esc_html_e( 'Reason', 'sabri-complete-home-news-feed' ); ?></dt><dd><?php echo esc_html( isset( $reason_labels[ $report['reason'] ] ) ? $reason_labels[ $report['reason'] ] : $report['reason'] ); ?></dd>
					<dt><?php esc_html_e( 'Reporter', 'sabri-complete-home-news-feed' ); ?></dt><dd><?php echo esc_html( $report['reporter_name'] . ' — ID ' . (int) $report['reporter_user_id'] ); ?></dd>
					<dt><?php esc_html_e( 'Reporter note', 'sabri-complete-home-news-feed' ); ?></dt><dd><?php echo '' !== $report['reporter_note'] ? nl2br( esc_html( $report['reporter_note'] ) ) : esc_html__( 'No additional details.', 'sabri-complete-home-news-feed' ); ?></dd>
					<dt><?php esc_html_e( 'Created', 'sabri-complete-home-news-feed' ); ?></dt><dd><?php echo esc_html( $report['created_at'] . ' UTC' ); ?></dd>
				</dl>
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="sabri-feed-report-update">
					<input type="hidden" name="action" value="<?php echo esc_attr( \Sabri\HomeNewsFeed\ReportAdmin::ACTION ); ?>" /><input type="hidden" name="report_id" value="<?php echo esc_attr( (string) $report['id'] ); ?>" /><?php wp_nonce_field( \Sabri\HomeNewsFeed\ReportAdmin::ACTION ); ?>
					<label><span><?php esc_html_e( 'Status', 'sabri-complete-home-news-feed' ); ?></span><select name="report_status" required><?php foreach ( $allowed_targets as $state_key ) : ?><option value="<?php echo esc_attr( $state_key ); ?>" <?php selected( $report['status'], $state_key ); ?>><?php echo esc_html( isset( $state_labels[ $state_key ] ) ? $state_labels[ $state_key ] : $state_key ); ?></option><?php endforeach; ?></select></label>
					<label><span><?php esc_html_e( 'Private moderator note', 'sabri-complete-home-news-feed' ); ?></span><textarea name="moderator_note" rows="4" maxlength="<?php echo esc_attr( (string) \Sabri\HomeNewsFeed\ReportPolicy::MODERATOR_NOTE_MAX ); ?>"><?php echo esc_textarea( $report['moderator_note'] ); ?></textarea></label>
...
<?php if ( ! empty( $data['max_pages'] ) && (int) $data['max_pages'] > 1 ) : ?>
	<nav class="tablenav-pages" aria-label="<?php esc_attr_e( 'Report queue pages', 'sabri-complete-home-news-feed' ); ?>">
		<?php for ( $page_number = 1; $page_number <= (int) $data['max_pages']; $page_number++ ) : ?>
			<?php $page_url = add_query_arg( array( 'page' => \Sabri\HomeNewsFeed\ReportAdmin::PAGE_SLUG, 'report_status' => isset( $filters['status'] ) ? $filters['status'] : '', 'report_reason' => isset( $filters['reason'] ) ? $filters['reason'] : '', 'report_object_type' => isset( $filters['object_type'] ) ? $filters['object_type'] : '', 'report_page' => $page_number ), admin_url( 'admin.php' ) ); ?>
