...
final class Phase5NewsroomAdmin {
	...
	public static function render(){if(!function_exists('current_user_can'))return;$page=isset($_GET['page'])?sanitize_key(wp_unslash($_GET['page'])):'';$slug=str_replace('sabri-newsroom-','',$page);$cap=self::capability($slug);$user_id=function_exists('get_current_user_id')?(int)get_current_user_id():0;if($user_id<=0||!CanonicalIdentityAdapter::current_action_ready($user_id)||!current_user_can($cap)){wp_die(esc_html__('You are not allowed to access this Newsroom screen.','sabri-complete-home-news-feed'));}$label=self::$screens[$slug]??'Newsroom';$diagnostics=Phase5Diagnostics::report();echo '<div class="wrap sabri-phase5-admin"><h1>'.esc_html__($label,'sabri-complete-home-news-feed').'</h1>';if('phase5-overview'===$slug||'phase5-release'===$slug||'phase5-system-check'===$slug){self::diagnostic_cards($diagnostics);}else{echo '<p>'.esc_html__('This bounded operational screen uses authenticated REST services and server-side policy checks.','sabri-complete-home-news-feed').'</p><div id="sabri-phase5-admin-app" data-screen="'.esc_attr($slug).'" aria-live="polite"></div>';}echo '</div>';}
