<?php
/**
 * Phase 3D private Following list runtime.
 *
...
/**
 * Provides a server-rendered current-user-only Following list.
 */
final class FollowingRuntime {
	const SHORTCODE = 'sabri_following';
	...
	/**
	 * Render the private Following list.
	 *
	...
	public static function render( $atts = array() ) {
		if ( ! Phase3FeatureSettings::enabled( 'follows_enabled' ) ) {
			return FeedRenderer::template( 'feed-error', array( 'message' => __( 'Following is currently unavailable.', 'sabri-complete-home-news-feed' ) ) );
		}
		...
		if ( $user_id <= 0 ) {
			...
			return FeedRenderer::template( 'following-list', array( 'logged_in' => false, 'login_url' => $login_url, 'items' => array() ) );
		}

		$result = FollowService::following_for_user( $user_id, $limit );
		$items  = ! empty( $result['ok'] ) && isset( $result['data']['items'] ) && is_array( $result['data']['items'] ) ? $result['data']['items'] : array();
		return FeedRenderer::template( 'following-list', array( 'logged_in' => true, 'login_url' => '', 'items' => $items ) );
	}
}
