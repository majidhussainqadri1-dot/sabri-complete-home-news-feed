...
final class FeedRenderer {
	...
	private static function feed_url( $mode, $page ) {
		...
		if ( function_exists( 'add_query_arg' ) ) {
			return add_query_arg( array( 'sabri_feed_mode' => sanitize_key( $mode ), 'sabri_feed_page' => $page ), $base );
		}
	...
	private static function rest_feed_url( $mode, $page, $per_page ) {
		...
		if ( function_exists( 'add_query_arg' ) ) {
			return add_query_arg( array( 'mode' => sanitize_key( $mode ), 'page' => (int) $page, 'per_page' => (int) $per_page ), $base );
		}
	...
	private static function post_excerpt( $post_id ) {
		if ( function_exists( 'get_the_excerpt' ) ) { return get_the_excerpt( $post_id ); }
	...
	private static function author_name( $post_id ) {
		...
		return ! empty( $projection['name'] ) ? (string) $projection['name'] : __( 'Sabri member', 'sabri-complete-home-news-feed' );
	}
	...
	private static function term_links( $post_id, $taxonomy ) {
		...
		$terms = get_the_terms( $post_id, $taxonomy ); if ( ! is_array( $terms ) ) { return array(); }
		$labels = array(); foreach ( $terms as $term ) { if ( is_object( $term ) && ! empty( $term->name ) ) { $labels[] = $term->name; } }
		return array_values( array_unique( $labels ) );
	}
	private static function medical_disclaimer( $type, array $settings ) {
		if ( empty( $settings['feed']['show_disclaimer'] ) || ! in_array( $type, array( 'clinical-case', 'research', 'public-health' ), true ) ) { return ''; }
