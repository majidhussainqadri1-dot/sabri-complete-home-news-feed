...
final class CompanionIntegrationRegistry {
	...
	public static function definitions() {
		return array(
			'membership' => array(
				'label' => 'Membership Core',
				'constants' => array( 'SABRI_MEMBERSHIP_CORE_VERSION' ),
				'classes' => array( 'Sabri_Membership_Core', 'Sabri\\Membership\\Plugin' ),
			),
			'profiles' => array(
				'label' => 'Profiles and Doctors',
				'shortcodes' => array( 'sabri_founder_profile', 'sabri_member_profile' ),
			),
			'shell' => array(
				'label' => 'Unified Application Shell',
				'constants' => array( 'SABRI_SHELL_VERSION' ),
				'classes' => array( 'Sabri\\UnifiedShell\\Plugin' ),
				'shortcodes' => array( 'sabri_shell_home_feed' ),
			),
			'notifications' => array(
				'label' => 'Unified Notifications',
				'constants' => array( 'SABRI_UNIFIED_NOTIFICATIONS_VERSION', 'SUN_VERSION' ),
				'classes' => array( 'Sabri\\Notifications\\Plugin' ),
				'shortcodes' => array( 'sabri_notifications' ),
				'functions' => array( 'sabri_notifications_render' ),
				'hooks' => array( 'sun_notify' ),
			),
			'network' => array(
				'label' => 'Network and Messaging',
				'constants' => array( 'SABRI_NETWORK_VERSION' ),
				'classes' => array( 'Sabri\\Network\\Plugin' ),
				'shortcodes' => array( 'sabri_network', 'sabri_messages' ),
				'functions' => array( 'sabri_network_render', 'sabri_messages_render' ),
			),
			'appointments' => array(
				'label' => 'Worldwide Clinic and Appointments',
				'constants' => array( 'SWC_VERSION', 'SABRI_WORLDWIDE_CLINIC_VERSION' ),
				'classes' => array( 'Sabri\\WorldwideClinic\\Plugin' ),
				'shortcodes' => array( 'swc_worldwide_clinic', 'swc_request_appointment', 'swc_my_appointments', 'swc_doctor_appointments', 'sabri_appointments' ),
				'functions' => array( 'swc_render_worldwide_clinic', 'sabri_appointments_render' ),
			),
			'marketplace' => array(
				'label' => 'Marketplace',
				'constants' => array( 'SABRI_MARKETPLACE_VERSION' ),
				'classes' => array( 'Sabri\\Marketplace\\Plugin' ),
				'shortcodes' => array( 'sabri_marketplace' ),
				'functions' => array( 'sabri_marketplace_render' ),
				'table_suffixes' => array( 'sabri_marketplace_products', 'sabri_marketplace_sellers' ),
			),
			'legacy_feed' => array(
				'label' => 'Legacy Social News and Publications',
				'constants' => array( 'SNP_VERSION' ),
				'classes' => array( 'SNP_Plugin' ),
				'shortcodes' => array( 'sabri_platform_home', 'sabri_news_feed', 'sabri_news_home' ),
				'post_types' => array( 'snp_publication' ),
			),
			'learning' => array(
				'label' => 'Learning',
				'shortcodes' => array( 'slc_learning_home', 'sabri_learning' ),
			),
			'encyclopedia' => array(
				'label' => 'Encyclopedia',
				'shortcodes' => array( 'he_encyclopedia_home', 'sabri_encyclopedia' ),
			),
			'doctors' => array(
				'label' => 'Doctors Directory',
				'shortcodes' => array( 'sdd_doctors_directory', 'sabri_doctors' ),
			),
			'video_wall' => array(
				'label' => 'Video Wall',
				'shortcodes' => array( 'svw_video_wall', 'sabri_video_wall' ),
			),
			'reels' => array(
				'label' => 'Reels',
				'shortcodes' => array( 'srl_reels', 'sabri_reels' ),
			),
			'pdf_library' => array(
				'label' => 'PDF Library',
				'shortcodes' => array( 'spl_library', 'sabri_pdf_library' ),
			),
			'radar' => array(
				'label' => 'Radar',
				'shortcodes' => array( 'srf_radar', 'sabri_radar' ),
			),
			'ai' => array(
				'label' => 'Sabri Classical Homeopathy AI',
				'shortcodes' => array( 'sai_study_guide', 'sabri_ai' ),
			),
		);
	}
	...
	private static function detect_definition( $key, array $definition ) {
		...
		foreach ( isset( $definition['table_suffixes'] ) ? $definition['table_suffixes'] : array() as $suffix ) {
			if ( self::table_exists( $suffix ) ) {
				$evidence[] = 'table:' . $suffix;
			}
		}
		$status = empty( $evidence ) ? 'Missing' : 'Connected';
		if ( 'Missing' === $status && ! empty( $definition['shortcodes'] ) ) {
