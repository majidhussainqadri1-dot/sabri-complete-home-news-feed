...
final class InteractionPermissions {
	...
	public static function can_interact_with_post( $post_id, $user_id = 0 ) {
		...
		if ( self::is_news_post( $post_id ) ) {
			$post = function_exists( 'get_post' ) ? get_post( $post_id ) : $post_id;
			if ( ! NewsPolicy::is_public_post( $post, 'single' ) || 'retracted' === NewsPolicy::workflow_state( $post_id ) ) { return false; }
