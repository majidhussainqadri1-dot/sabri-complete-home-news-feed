...
final class Phase4Contracts {
	...
	public static function feature_enabled( $feature, $settings = null ) {
		$feature = self::sanitize_identifier( $feature );
		...
		return is_array( $settings ) && isset( $settings[ $feature ] ) && 1 === (int) $settings[ $feature ];
	}
	...
	/**
	 * WordPress storage status for each domain workflow state.
	 *
	 * The complete domain state is always retained in WORKFLOW_META_KEY. This
	 * avoids truncation because WordPress stores post_status in varchar(20).
	 */
	...
	/** Sanitize an internal identifier without trusting WordPress availability. */
