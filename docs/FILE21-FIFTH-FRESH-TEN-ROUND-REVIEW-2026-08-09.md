...
| 10 | Fresh exact-head CI closure and regression compatibility after all preceding fixes | DEFECT | Exact-head CI first exposed the historical third-review executable test, and PR validation then exposed the historical fourth-review workflow, both still asserting the previous File 04/20/24 pins. Updated both continuing gates so historical round classifications stay immutable while current compatibility assertions follow the latest approved immutable companion heads. Final merge remains prohibited until the complete PR exact-head suite is green. |
