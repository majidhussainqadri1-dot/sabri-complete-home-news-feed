...
namespace {
	...
	function wp_generate_uuid4(): string { static $i = 100; ++$i; return sprintf( '00000000-0000-4000-8000-%012d', $i ); }
	...
	function get_post_meta( int $post_id, string $key, bool $single = false ): mixed { $value = $GLOBALS['file21_test_post_meta'][ $post_id ][ $key ] ?? ''; return $single ? $value : ( '' === $value ? array() : array( $value ) ); }
	function update_post_meta( int $post_id, string $key, mixed $value ): int { $GLOBALS['file21_test_post_meta'][ $post_id ][ $key ] = $value; return 1; }
	function delete_post_meta( int $post_id, string $key, mixed $value = '' ): bool { if ( ! isset( $GLOBALS['file21_test_post_meta'][ $post_id ][ $key ] ) ) { return false; } if ( '' !== $value && $GLOBALS['file21_test_post_meta'][ $post_id ][ $key ] !== $value ) { return false; } unset( $GLOBALS['file21_test_post_meta'][ $post_id ][ $key ] ); return true; }
	function get_posts( array $args ): array {
		...
		foreach ( $GLOBALS['file21_test_posts'] as $post_id => $post ) {
			...
			foreach ( (array) ( $args['meta_query'] ?? array() ) as $clause ) {
				...
				if ( ( $GLOBALS['file21_test_post_meta'][ $post_id ][ $clause['key'] ] ?? null ) !== ( $clause['value'] ?? null ) ) { $match = false; }
			}
...
namespace Sabri\HomeNewsFeed {
	...
	final class Composer {
		public static function create_or_update_from_request( array $input, array $files = array(), int $user_id = 0 ): array {
			...
			return array( 'ok' => true, 'post_id' => $post_id, 'status' => $status );
		}
	}
	...
	$assert( 'sabri_feed_create_posts' === $adapter->required_capability(), 'Native File 21 workflow adapter must expose its exact create capability.' );
	...
	$GLOBALS['file21_test_current_user'] = 1;
	$assert( isset( $adapter->schema()['fields']['feed_type']['choices']['founder_update'] ), 'Founder schema omits Founder Update.' );

	$calls_before_direct = (int) $GLOBALS['file21_test_composer_calls'];
	$direct = $adapter->submit( 1, $key, $payload );
	$assert( 'invalid_reference' === $error_code( $direct ), 'Final submission without a native draft reference was accepted.' );
	$assert( $calls_before_direct === (int) $GLOBALS['file21_test_composer_calls'], 'Reference-less final submission invoked the Composer.' );
	...
	$submit_payload = array_merge( $payload, array( 'native_reference' => $draft_ref ) );
	$preview = $adapter->preview( 1, $submit_payload );
	...
	$assert( 'conflict' === $error_code( $adapter->preview( 1, $submit_payload ) ), 'Pending moderation post was accepted by preview.' );
	...
	$assert( $recovered === $adapter->submit( 1, $key, $submit_payload ), 'Completed replay did not return the native result.' );
	$assert( $before_submit + 1 === (int) $GLOBALS['file21_test_composer_calls'], 'Completed replay invoked the Composer.' );
	$assert( 'conflict' === $error_code( $adapter->submit( 1, $key, array_merge( $submit_payload, array( 'content' => 'Different content' ) ) ) ), 'Same key with different payload did not conflict.' );

	$concurrent_key = '00000000-0000-4000-8000-000000000005:00000000-0000-4000-8000-000000000006';
	$concurrent_draft = $adapter->create_draft( 1, null, $payload );
	...
	UniversalComposerWorkflowStore::acquire_record( $concurrent_option, 1, $concurrent_hash, $concurrent_fingerprint );
	$record = UniversalComposerWorkflowStore::load_record( $concurrent_option );
	UniversalComposerWorkflowStore::persist_processing_reference( $concurrent_option, is_array( $record ) ? $record : array(), $concurrent_id );
	$calls_before_concurrent = (int) $GLOBALS['file21_test_composer_calls'];
	$assert( 'temporarily_unavailable' === $error_code( $adapter->submit( 1, $concurrent_key, $concurrent_payload ) ), 'Active processing lease was ignored.' );
	$assert( $calls_before_concurrent === (int) $GLOBALS['file21_test_composer_calls'], 'Active processing lease allowed a second Composer call.' );

	$record = UniversalComposerWorkflowStore::load_record( $concurrent_option );
	$record['state'] = 'recoverable';
	$record['expires_at'] = time() + 600;
	update_option( $concurrent_option, $record, false );
	$GLOBALS['file21_test_options'][ 'sabri_hnf_file22_exec_1_' . $concurrent_hash ] = array( 'token' => 'other-request', 'expires_at' => time() + 120 );
	$assert( 'temporarily_unavailable' === $error_code( $adapter->submit( 1, $concurrent_key, $concurrent_payload ) ), 'Active execution lock was ignored.' );
	$assert( $calls_before_concurrent === (int) $GLOBALS['file21_test_composer_calls'], 'Active execution lock allowed a Composer call.' );

	$private_draft = $adapter->create_draft( 1, null, array_merge( $payload, array( 'visibility' => 'private' ) ) );
	$private_ref = is_array( $private_draft ) ? (string) $private_draft['native_reference'] : '';
	$private_key = '00000000-0000-4000-8000-000000000003:00000000-0000-4000-8000-000000000004';
	$private = $adapter->submit( 1, $private_key, array_merge( $payload, array( 'visibility' => 'private', 'native_reference' => $private_ref ) ) );
	$assert( '' !== $adapter->canonical_url( 1, is_array( $private ) ? (string) $private['native_reference'] : '' ), 'Owner cannot resolve private canonical URL.' );
	$assert( '' === $adapter->canonical_url( 2, is_array( $private ) ? (string) $private['native_reference'] : '' ), 'Private canonical URL leaked cross-user.' );
