<?php
/**
 * Phase 4B newsroom, composer, queue, workflow, scheduling, and audit tests.
 *
...
use Sabri\HomeNewsFeed\NewsAudit;
...
global $sabri_test_current_user_id, $sabri_test_user_roles, $sabri_test_scheduled_events;
...
sabri_phase4b_assert( '2027-01-01 22:04:05' === $valid['data']['schedule_at_utc'], 'Schedule values must normalize to UTC.' );
...
foreach ( array( 'own-drafts', 'editorial-review', 'fact-check', 'medical-review', 'changes-requested', 'approved', 'scheduled', 'publication-ready', 'published', 'accountability' ) as $queue ) {
...
sabri_phase4b_assert( 77 === (int) get_post_meta( $post_id, '_thumbnail_id', true ), 'Authorized featured image selection must use core thumbnail storage.' );
...
$scheduled = NewsSchedulingService::schedule( $post_id, '2027-01-02T03:04:05+05:00' );
...
$scheduled_again = NewsSchedulingService::schedule( $post_id, '2027-01-02T03:04:05+05:00' );
...
sabri_phase4b_assert( is_array( $audit_option ) && count( $audit_option ) >= 4, 'Consequential editorial actions must append private audit events.' );
...
$blocked = NewsService::transition( $post_id, 'ready-for-publication', array( 'method' => 'POST', 'nonce_verified' => true ) );
...
echo "OK - Phase 4B newsroom, composer, queue, workflow, scheduling, and audit tests passed.\n";
