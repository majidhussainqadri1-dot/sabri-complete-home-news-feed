...
sabri_file21_completion_assert( false !== strpos( $timeline, "sabri_file21_profile_timeline_provider_health_v1" ), 'File 21 must publish the exact File 03 timeline health contract.' );
sabri_file21_completion_assert( false !== strpos( $timeline, "sabri_file21_profile_timeline_items_v1" ), 'File 21 must publish the exact File 03 timeline item contract.' );
sabri_file21_completion_assert( false !== strpos( $timeline, "FILE03_PROVIDER_CONTRACT" ) && false !== strpos( $timeline, "FILE03_PROVIDER_MAX" ), 'File 03 timeline projection must be versioned and bounded.' );
sabri_file21_completion_assert( false !== strpos( $timeline, 'PostMetadata::user_can_view( $post_id, $viewer )' ) && false !== strpos( $timeline, "get_post_field( 'post_author', $post_id )" ), 'File 03 timeline projection must revalidate viewer authorization and author ownership.' );
