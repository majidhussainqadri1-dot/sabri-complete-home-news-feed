...
sabri_phase3_assert( '1.0.1' === $identity['version'], 'Phase 3 contracts must remain compatible with the harmonized File 21 runtime version.' );
...
$expected_flags = array( 'reactions_enabled', 'dislikes_enabled', 'comments_enabled', 'saves_enabled', 'follows_enabled', 'followers_visibility_enabled', 'reports_enabled', 'polls_enabled', 'notification_bridge_enabled', 'view_logging_enabled' );
...
sabri_phase3_assert( 1000 === $settings['moderation']['reporter_note_max_length'], 'Reporter note contract must be bounded to 1,000 characters.' );
sabri_phase3_assert( 2000 === $settings['moderation']['moderator_note_max_length'], 'Moderator note contract must be bounded to 2,000 characters.' );
sabri_phase3_assert( Phase3Contracts::report_states() === $settings['moderation']['allowed_report_states'], 'Report moderation states must remain frozen in settings.' );
...
$expected_routes = array( 'engagement', 'reaction_create', 'reaction_delete', 'comments', 'comment_create', 'comment_update', 'comment_delete', 'save_create', 'save_delete', 'my_saves', 'follow_create', 'follow_delete', 'my_following', 'report_create', 'report_queue', 'report_update', 'poll_vote', 'poll_vote_delete', 'poll_results' );
...
if ( $phase3_failures ) {
	echo "FAILED\n";
	foreach ( $phase3_failures as $failure ) { echo '- ' . $failure . "\n"; }
	exit( 1 );
}
