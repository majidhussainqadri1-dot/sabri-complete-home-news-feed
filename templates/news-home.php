...
<main id="main-content" class="sabri-news sabri-news-home">
	...
	<?php else : ?>
		<?php foreach ( $components as $component ) : ?>
			<?php $items = ! empty( $component['items'] ) && is_array( $component['items'] ) ? $component['items'] : array(); ?>
			<?php if ( ! $items && ! in_array( $component['key'], array( 'featured', 'latest' ), true ) ) { continue; } ?>
			<section class="sabri-news-home__section sabri-news-home__section--<?php echo esc_attr( sanitize_html_class( $component['key'] ) ); ?>" aria-labelledby="sabri-news-section-<?php echo esc_attr( sanitize_html_class( $component['key'] ) ); ?>">
				<header class="sabri-news-home__section-header">
					<h2 id="sabri-news-section-<?php echo esc_attr( sanitize_html_class( $component['key'] ) ); ?>"><?php echo esc_html( $component['title'] ); ?></h2>
				...
				<?php if ( $items ) : ?>
					<div class="sabri-news-grid<?php echo 'featured' === $component['key'] ? ' sabri-news-grid--featured' : ''; ?>">
