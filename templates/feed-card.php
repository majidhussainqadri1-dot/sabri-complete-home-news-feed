...
$author_projection = class_exists( '\\Sabri\\HomeNewsFeed\\CanonicalIdentityAdapter' )
	? \Sabri\HomeNewsFeed\CanonicalIdentityAdapter::public_projection( $author_id )
	: array();
...
<article class="sabri-hnf-card" id="sabri-hnf-post-<?php echo esc_attr( $post_id ); ?>">
		...
		<div class="sabri-hnf-card__byline">
			<div class="sabri-hnf-card__author">
				<?php if ( '' !== $author_profile_url ) : ?>
					<a href="<?php echo esc_url( $author_profile_url ); ?>"><?php echo esc_html( $author_name ); ?></a>
				<?php else : ?>
	...
	<?php if ( ! empty( $topics ) || ! empty( $categories ) || ! empty( $hashtags ) ) : ?>
		<footer class="sabri-hnf-card__terms">
			<?php foreach ( array_merge( $topics, $categories, $hashtags ) as $term_label ) : ?><span><?php echo esc_html( $term_label ); ?></span><?php endforeach; ?>
		</footer>
	<?php endif; ?>
