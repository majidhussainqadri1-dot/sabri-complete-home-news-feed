...
foreach ($relativePath in $requiredRuntimeFiles) {
	...
	if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) { throw "Required runtime repair file is missing: $relativePath" }
}
...
if (Test-Path -LiteralPath $stageDir) {
	...
	if (-not $resolvedStage.StartsWith($resolvedRelease, [System.StringComparison]::OrdinalIgnoreCase)) { throw "Refusing to remove a staging path outside the release directory: $resolvedStage" }
...
Get-ChildItem -LiteralPath $Root -Force | ForEach-Object {
	if ($excludedDirs -contains $_.Name -or $excludedFiles -contains $_.Name -or $_.Name -like '*.log') { return }
	...
	} else { Copy-Item -LiteralPath $_.FullName -Destination $destination -Force; $copied++ }
}
foreach ($relativePath in $requiredRuntimeFiles) {
	$stagedPath = Join-Path $topDir $relativePath
	if (-not (Test-Path -LiteralPath $stagedPath -PathType Leaf)) { throw "Required runtime repair file was not staged: $relativePath" }
}
foreach ($path in @($zipPath, $shaPath, $reportPath)) { if (Test-Path -LiteralPath $path) { Remove-Item -LiteralPath $path -Force } }
...
$report = @(
	'# Sabri Complete Home and News Feed Phase 4C Candidate Test Report','',
	...
	'- Package status: exact-head WordPress Playground candidate only; not approved for version promotion or live deployment.',
	'- Required real integration matrix: WordPress latest/PHP 8.3 and WordPress 6.8/PHP 8.1.',
	'- Phase 4C coverage: approved public snapshots, private correction staging, landing/archive/single/taxonomy routes, bounded search, dedicated News cards, Home Feed integration, Phase 3 interaction boundary, read-only REST, cache/privacy boundaries.',
