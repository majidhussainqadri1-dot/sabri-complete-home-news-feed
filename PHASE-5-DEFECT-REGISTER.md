...
| P5-D012 | High | v2 run `30158855890` completed all 24 cycles and 7,200 seconds, and its tracked manifests matched, but cycle package SHA values remained empty. The cycle body was piped to `tee`, causing Bash to execute the grouped commands in a subshell; `LAST_PACKAGE` and `LAST_PACKAGE_SHA` assignments made by `verify_fresh_package()` were lost before cycle evidence and final-package retention. | Resilient v3 workflow added in `0b8c037c52f143973d8943b0642b7b71339b5cd0`; v2 retired in `116243b8dd33a3a635fe4f9e3faff30ab2f5efe9`. v3 uses process-substitution logging so state remains in the current shell and asserts a non-empty 64-character SHA in every cycle and final result. | Ordinary acceptance and the complete v3 two-hour acceptance restarted from zero. Acceptance requires exactly 24 cycle rows each containing a 64-character package SHA, 7,200 seconds, matching manifests, final package checksum, and both final packaged matrices. | Closed; v2 evidence invalidated |
| P5-D013 | High | On exact head `a95323faffc26a65180b80dad5bc01145abdb10f`, legacy runs `30163318051` and `30163318044` still expected the frozen Phase 3 package filename, while the canonical Phase 5 builder emitted only the Phase 5 candidate filename. The build workflow therefore failed ZIP-structure verification, and both packaged Playground jobs failed before WordPress installation because the configured ZIP mount source did not exist. Runtime/source Playground tests and the canonical Phase 5 package itself were otherwise successful. | `tools/build-release.ps1` correction `89a3ec7b53b94290a152cc891dbce9f2945d7645` now emits a byte-identical, checksum-verified legacy filename alias alongside the canonical Phase 5 candidate without changing plugin contents, version, schema, gates, or publication behavior. | Replacement legacy Build/Test and Packaged workflows, ordinary Phase 5 acceptance, and the complete two-hour acceptance must all restart from zero on the new exact head; both package filenames must resolve to the same SHA-256 and the packaged installer lifecycle must pass on WordPress 6.8/PHP 8.1 and latest/PHP 8.3. | Closed; prior exact-head evidence invalidated |

## Current open defects

No known Critical, High, Medium, Low, or unclassified implementation defect is open. The new exact head must independently pass all ordinary, legacy packaged/build, and complete v3 two-hour acceptance; evidence from earlier exact heads is not combined with the restarted run.
